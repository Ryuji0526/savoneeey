...
describe('components/user/UserEdit.vue', () => {
  let vuetify
  let wrapper
  let spyEditUser
  let observer
  ...
  beforeEach(() => {
    ...
    spyEditUser = jest.spyOn(UserEdit.methods, 'edit')
  ...
  describe('動作確認', () => {
    test('passwordBtnを押すとrevealがtrueになる', async () => {
      ...
      wrapper.find('[data-testid="passwordBtn"]').trigger('click')
  ...
  describe('バリデーション', () => {
    ...
    describe('正しい入力', () => {
      beforeEach(async () => {
        ...
        emailField.setValue('sample@sample.com')
        await observer.validate()
      })
      test('エラーが表示されない', () => {
